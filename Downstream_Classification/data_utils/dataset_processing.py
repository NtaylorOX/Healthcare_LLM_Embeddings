...
def encode_classes(df,  label_col = None, sort_by_value_count = False):
    
    ''' 
    Function to encode non-integer categories or classes into integers or floats to be used by downstream models
    '''
    
    
    # get classes as unique/set of the label col - we sort to make int labels stay in order if possible
    #TODO add the sorting by value_counts rather than default
    if sort_by_value_count:
        classes = list(df[label_col].value_counts().keys()) 
    else:
...
def convert_to_binary_classes(df, split_value, label_col = "label"):
    ...
    '''
    Function to take a dataframe with a class label with range 0-N where N > 1 i.e. more than 2 classes and convert to a binary 0/1 class problem given a split value in range 0-N.
    '''
    
    assert split_value <= df[label_col].max() and split_value >= df[label_col].min(), f"it appears that the split value given:{split_value} is outside of the range of the dfs label min:{df[label_col].min()} and max:{df[label_col].max()}"
...
# below is for a dataset dictionary style from OpenPrompt



class FewShotSampler_dataset(object):
    '''
