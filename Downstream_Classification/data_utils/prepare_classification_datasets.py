...
'''
Script to prepare incident notes data for different classification tasks based on the available categorical variables including: location, severity, type.
The datasets can be processed and stored individually in their own respective folder, or all together in one big dataset. These just make certain downstream tasks easier to organise.
...
class LMTextData:
    def __init__(self,
                ...
                seed = 41,
                text_col = None,                           
                dataset_names_map = {"Key.PD09":"severity","Key.IN05":"type","Key.RP02":"location"},
    ...
    def clean_data(self, admin_language,                   
        ...
        Args:
            ...
            remove_punctuation: bool -> False will not remove punctuationm, true will use regex to strip certain punctuation 
       ...
       Returns: pd.DataFrame: notes_df
        ...
        if remove_punctuation:
            filtered_df[text_col] = filtered_df[text_col].replace(r"\[.*?\]", "", regex=True)
    ...
    def create_classification_datasets(self):
        ...
        '''
        Slightly crude function to run over the dataframe and create each classification dataset we have
        ...
        if self.sample:
            logger.info(f"Will be sampling the dataset")
        ...
        # clean notes
        cleaned_all_df = self.clean_data(admin_language=self.admin_language, 
        ...
        if self.process_individually:
            ...
            for key, dataset in self.dataset_names_map.items():           
                ...
                logger.info(f"Shape of training data: {df_train.shape}\n\nShape of validation data: {df_valid.shape}\n\nShape of test data: {df_test.shape}")
...
def main():
    ...
    parser.add_argument("--replacement_map",
                        ...
                        type = list,
                        help = "set of tuples to act as replacement maps. The first element of each tuple will be replaced by the second iterativelys")
    ...
    parser.add_argument("--dataset_names_map",
                        default = {"Key.PD09":"severity","Key.IN05":"type","Key.RP02":"location"},
                        type=dict,
                        help = "The mapping from key to a more generic class name")
    parser.add_argument("--process_individually",
                        action = "store_true",
                        help = "Whether or not to process and save each class/dataset separately into own subfolders or put all in one")
