...
class IncidentDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        #   return_tensors='pt',
        # )
        encoding = self.tokenizer(all_text, padding = "max_length", truncation = True, max_length = self.max_token_len, return_tensors = "pt" )
...
class IncidentModel(pl.LightningModule):
    def __init__(self,
        ...
        super().__init__()
        logger.warning(f"Building model based on following architecture. {model} and number of labels: {num_labels}")
        
        self.save_hyperparameters()
        
        self.num_labels = num_labels        
        # this will determine if we use the transformers autoforsequenceclassification class or a custom version with our own classifier
        self.model_type = model_type
        
        if self.model_type == "autoforsequence":
            logger.warning(f"Will be using AutoModelForSequenceClassification!")
            self.model = AutoModelForSequenceClassification.from_pretrained(f"{model}", cache_dir = cache_dir, num_labels = self.num_labels, return_dict=True)
        elif self.model_type == "customclassifier":
            
            logger.warning(f"Will be using based AutoModel with our own classification head! Be warned this can cause problems for many frameworks that expect transformer based classes in their purest form")
            self.model = AutoModel.from_pretrained(f"{model}", cache_dir = cache_dir, return_dict=True)
            # can also add the automodelforsequence classification here - may be easier for later use
            # self.model = AutoModelForSequenceClassification.from_pretrained(f"{model}", cache_dir = cache_dir, return_dict=True)
    ...
    def unfreeze_encoder(self) -> None:
        ...
        if self._frozen:
            print(f"Model was frozen so will try unfreezing!")
            if self.model_type == "autoforsequence":
    ...
    def forward(
        ...
        if self.model_type == "autoforsequence":
            ...
            if not return_dict:
                raise NotImplementedError
                # output = (logits,) + outputs[2:] # this is from original modelling_roberta - it may break the pytorch training setup in its current form
    ...
    def validation_epoch_end(self, outputs):
        ...
        for output in outputs:
            ...
            for out_predictions in output["predictions"]:
                ...
                # append probas
                    ...
                    # just get the proba for the 1 class as this is the format required for sklearn roc calc for binary
        ...
        logger.info(f"predictions are: {predictions} with the set being : {set(predictions)}")
        
        logger.warning(f"min of labels is: {min(labels)} and max is: {max(labels)}")
        logger.warning(f"min of predictions is: {min(predictions)} and max is: {max(predictions)}")
